...
EXPORT void torusPolynomialMultNaive(TorusPolynomial* result, const IntPolynomial* poly1, const TorusPolynomial* poly2) {
    torusPolynomialMultNaive_aux(result->coefsT, poly1->coefs, poly2->coefsT, Value_N);
}
...
EXPORT void torusPolynomialMultKaratsuba(TorusPolynomial* result, const IntPolynomial* poly1, const TorusPolynomial* poly2){
	Torus32* R = new Torus32[2*Value_N-1];
	char* buf = new char[16*Value_N]; //that's large enough to store every tmp variables (2*2*N*4)

	// Karatsuba
	Karatsuba_aux(R, poly1->coefs, poly2->coefsT, Value_N, buf);

	// reduction mod X^N+1
	for (int32_t i = 0; i < Value_N-1; ++i)
	    result->coefsT[i] = R[i] - R[Value_N+i];
	result->coefsT[Value_N-1] = R[Value_N-1];

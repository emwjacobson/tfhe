...
static TFheGateBootstrappingParameterSet *default_gate_bootstrapping_parameters() {
    LweParams *params_in = new_LweParams(PARAM_n, PARAM_ks_stdev, PARAM_max_stdev);
    TLweParams *params_accum = new_TLweParams(PARAM_N, PARAM_k, PARAM_bk_stdev, PARAM_max_stdev);
    TGswParams *params_bk = new_TGswParams(PARAM_bk_l, PARAM_bk_Bgbit, params_accum);

    TfheGarbageCollector::register_param(params_in);
    TfheGarbageCollector::register_param(params_accum);
    TfheGarbageCollector::register_param(params_bk);

    return new TFheGateBootstrappingParameterSet(PARAM_ks_length, PARAM_ks_basebit, params_in, params_bk);
}

EXPORT TFheGateBootstrappingParameterSet *new_default_gate_bootstrapping_parameters() {
    return default_gate_bootstrapping_parameters();
}
